...
## Current low-level foundations

The host-validated kernel now exposes MMU-style map, protect, translate, and fault paths over the existing per-process page metadata. The HAL includes a portable IRQ registration and dispatch table so drivers can be tested without hard-coding board interrupt-controller registers. The network helpers construct UDP and IPv4 packets and preserve checksum/DNS utilities as the base for the larger TCP/IP stack.
